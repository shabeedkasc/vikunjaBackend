<template>
	<div class="content loader-container" :class="{'is-loading': loading}" v-cy="'projects-list'">
		...
		<span class="padding-right10">{{ $t('project.showArchived') }}</span>		
			</fancycheckbox>

			<div class="action-buttons ">
				<x-button :to="{name: 'filters.create'}" icon="filter">
					<span class="padding-right10">		{{ $t('filters.create.title') }}</span>
				</x-button>
				<x-button :to="{name: 'project.create'}" icon="plus" v-cy="'new-project'">
					<span class="padding-right10">	{{ $t('project.create.header') }}</span>
				</x-button>
			</div>
		</header>
