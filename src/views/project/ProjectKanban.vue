		...
		<template #default>
					...
					>
						<div class="bucket-header" @click="() => unCollapseBucket(bucket)">
							...
							>
								...
								>
								<span class="padding-right10">	{{ $t('project.kanban.doneBucket') }}</span>
								</dropdown-item>
								<dropdown-item
									...
									icon="th"
								>
								<span class="padding-right10">	{{ $t('project.kanban.defaultBucket') }}</span>
								...
								<dropdown-item
									...
									icon="angles-up"
								>
								<span class="padding-right10">	{{ $t('project.kanban.collapse') }}</span>
							...
							<template #footer>
								<div class="bucket-footer" v-if="canWrite">
									...
									<x-button
										...
										variant="secondary"
									>
									<span class="padding-right10">	{{ bucket.tasks.length === 0 ? $t('project.kanban.addTask') : $t('project.kanban.addAnotherTask') }}</span>
									</x-button>
								</div>
							</template>
			...
			<div class="bucket new-bucket" v-if="canWrite && !loading && buckets.length > 0">
				...
				<x-button
					...
					icon="plus"
				>
				<span class="padding-right10" >	{{ $t('project.kanban.addBucket') }}</span>
				</x-button>
			</div>
		</div>
