<template>
	<div class="is-max-width-desktop has-text-right" v-cy="'showTasks'">
		<h3 class="mb-2 title">
			{{ pageTitle }}
		</h3>
		<p v-if="!showAll" class="show-tasks-options">
			<datepicker-with-range @update:model-value="setDate">
				<template #trigger="{toggle}">
					<x-button @click.prevent.stop="toggle()" variant="primary" :shadow="false" >
						{{ $t('task.show.select') }}
					</x-button>
				</template>
			</datepicker-with-range>
		
			<fancycheckbox @update:model-value="setShowNulls" :model-value="showNulls">
				
				<span class="padding-right10"> {{ $t('task.show.noDates') }} </span>
			</fancycheckbox><fancycheckbox @update:model-value="setShowOverdue" :model-value="showOverdue">
				
				<span class="padding-right10"> {{ $t('task.show.overdue') }}</span>
			</fancycheckbox>
		</p>
